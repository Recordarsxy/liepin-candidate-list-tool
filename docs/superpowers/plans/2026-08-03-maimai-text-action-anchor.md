...
Replace the age-dependent `findNameBeforeAge` with `findProfileName(tokens)`. Its boundary is the earliest age, experience token such as `12年`, education level (`大专/本科/硕士/博士`), expectation marker, or `PERIOD`. Before that boundary, prefer the first explicit honorific or masked-name token; otherwise use the first short Chinese name candidate after excluding status text matching `活跃|求职|机会|动向|招聘动态|关注行情|简历`. For current location, prefer the first normalizable location after the visible education-level token and before expectation; otherwise scan after age, or after the resolved name when age is absent, while excluding experience, degree, name, and status tokens. Never use the name as a location.
